...
static int scalar_product(uint8_t* a, uint8_t* b, int size) { // Computing the scalar product of a and b, with "size" bytes length.
    int out = 0;
...
static int norm(uint8_t* a, int size){  // Computing the norm of a, with "size" bytes length.
    int out = 0;
    for (int i = 0; i < size; i++){
        for (int j = 0; j < CHAR_BIT; j++){
            if (((a[i] >> j) & 1U) == 1){
...
static void generate_random_key(uint8_t* buf){  // Generating a random key. The 2 last bytes are filled with 0.
...
int stego_gen_keys(void* buf, int count) {  // Generating "count" orthonormal keys. Uses special Gram-Schmidt to do so.
    ...
    for (int i = 0; i < total_keys_size; i+=key_size){ 
        ...
        for (int j = 0; j < i; j+=key_size){  // Add to "rnd_key" all the keys before him which have scalar product 1 with him. 
            int product = scalar_product(rnd_key, &keys[j], STEGO_KEY_BITS/CHAR_BIT);
            if (product == 1){
